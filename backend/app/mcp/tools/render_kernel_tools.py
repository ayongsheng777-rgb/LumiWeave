...
async def create_render(visual_intent: dict, capability_required: Optional[list[str]] = None):
    ...
    try:
        ...
        plan = await compile(visual_intent, capability_required)
        jm = get_job_manager()
...
async def query_render_status(job_id: str):
    ...
    jm = get_job_manager()
    try:
        resp = await jm.router.status("cloud", job_id)
        return {"ok": True, "data": {
            "job_id": job_id,
            "status": resp.status,
...
async def get_render_events(job_id: str):
    ...
    events = job_get_events(job_id)
    return {"ok": True, "data": events}


# ── WebSocket 端点 ────────────────────────────────────────────────────────────
_websocket_router = APIRouter()


@_websocket_router.websocket("/ws/render")
